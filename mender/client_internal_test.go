...
type MenderInternalTestSuite struct {
	suite.Suite
}

func Test_MenderInternalSuite(t *testing.T) {
	suite.Run(t, new(MenderInternalTestSuite))
}

func (ms *MenderInternalTestSuite) TestUpdateInventory() {
	req := ms.Require()
	asrt := ms.Assert()
	...
	for _, arg := range args {
		...
		client, err := New(
			WithServer(srv.URL, arg.teenantToken),
			WithSigner(keys),
			WithDevice(dev),
			WithDownloader(mocks.NewMockDownloader(ctrl)),
			WithInstaller(mocks.NewMockInstaller(ctrl)),
			WithRebooter(mocks.NewMockRebooter(ctrl)),
			WithLoadSaver(mocks.NewMockLoadSaver(ctrl)),
		)
...
func (ms *MenderInternalTestSuite) TestSendStatus() {
	req := ms.Require()
	...
	args := []struct {
		...
		},
		{
			...
			sendStatus:     PauseBeforeInstalling,
		...
		},
		{
			...
			sendStatus:     PauseBeforeRebooting,
		...
		},
		{
			name:           "NoContent",
			statusCode:     http.StatusNoContent,
			sendStatus:     PauseBeforeCommiting,
	...
	for _, arg := range args {
		...
		handle.Handle("/api/devices/v1/deployments/device/deployments/next", http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
			writer.WriteHeader(http.StatusOK)
			body, _ := json.Marshal(DeploymentInstructions{
				ID: arg.deployID,
		...
		client, err := New(
			WithServer(srv.URL, "teenant token"),
			WithSigner(keys),
			WithDevice(dev),
			WithDownloader(mocks.NewMockDownloader(ctrl)),
			WithInstaller(mocks.NewMockInstaller(ctrl)),
