...
func (f FakeDevice) Info() (device.Info, error) {
	return device.Info{
		DeviceType:   "fake device",
		ArtifactName: "release-v1",
	}, nil
}

func (f FakeDevice) ID() ([]device.Attribute, error) {
	return []device.Attribute{
		{
			Name:  "mac",
			Value: []string{"01:02:03:04:05:06"},
		},
	}, nil
}

func (f FakeDevice) Attributes() ([]device.Attribute, error) {
	return []device.Attribute{
		{
			Name:  "my awesome attribute",
			Value: []string{"attr"},
		},
	}, nil
}
